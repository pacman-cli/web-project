...
// Student message portal — fully dynamic SPA (no page reloads, realtime polling)
...
<style>
    ...
    .conversation-item.active { background-color: rgba(var(--tw-primary-rgb, 106 26 140) / 0.08); border-left: 4px solid var(--tw-secondary, #6a1a8c); }
...
function renderConversations() {
    ...
    if (conversations.length === 0) {
        container.innerHTML = '<p class="p-lg text-center text-on-surface-variant">No conversations yet. Enroll in a course to start chatting.</p>';
        return;
    }
    ...
    conversations.forEach(conv => {
        ...
        const avatarBg = conv.partner_role === 'instructor' ? 'bg-primary/10 text-primary' : 'bg-tertiary/10 text-tertiary';
        const roleBadge = conv.partner_role === 'instructor'
            ? '<span class="text-[9px] px-1 py-[1px] rounded bg-primary/10 text-primary font-bold uppercase">Instr</span>'
            : '<span class="text-[9px] px-1 py-[1px] rounded bg-tertiary/10 text-tertiary font-bold uppercase">Student</span>';
...
function selectConversation(courseId, partnerId) {
    ...
    document.getElementById('chat-header-content').innerHTML = `
        ...
        <div>
            ...
            <p class="text-xs text-secondary">${escapeHtml(conv.course_title)} · ${conv.partner_role === 'instructor' ? 'Instructor' : 'Classmate'}</p>
        </div>`;
